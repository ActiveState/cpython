...
# Does not seem to be in Python2
...
class UstarReadTest(ReadTest, unittest.TestCase):
    ...
    # FIX: This is a new test, things need to be done in the Python2 way
...
class MiscReadTestBase(CommonReadTest):
    ...
    # FIX: This needs to be converted to Python2
    ...
    # FIX: This is new and needs to be converted to Python2
    ...
    # FIX: This should already work, not sure whwy it can't find skip_unless_symlink
    ...
    # FIX: This is new, needs to be converted to Python2
...
# LZMA isn't in Python2 I believe
...
# FIX: Needs to be reworked for Python2, it existed previously
...
# FIX: Needs to be corrected for Python2
...
class WriteTest(WriteTestBase, unittest.TestCase):
    ...
    # FIX: I'm pretty sure this should be working
    # @support.skip_unless_symlink
    ...
    def test_exclude(self):
        ...
        try:
            for name in ("foo", "bar", "baz"):
                ...
                test.support.create_empty_file(name)
    ...
    def _test_pathname(self, path, cmp_path=None, dir=False):
        ...
        if not dir:
            test.support.create_empty_file(foo)
        else:
            os.mkdir(foo)
...
# FIX: Multiple changes, needs to be fixed
...
# FIX: ?We don't have the command line stuff in Python2
...
#         for filetype in (GzipTest, Bz2Test):
...
# FIX: Not sure if we have Mock or not in Python2
        ...
        for attr_names in ({'mtime'}, {'mode'}, {'uid'}, {'gid'},
            ...
            with self.subTest(attr_names=attr_names), \
                ...
                for expected, got in zip(tio_prev.detach().getvalue().split(),
                    ...
                    if attr_names == {'mtime'} and re.match(r'2003-01-\d\d', expected):
                        self.assertEqual(got, b'????-??-??')
                    elif attr_names == {'mtime'} and re.match(r'\d\d:\d\d:\d\d', expected):
                        self.assertEqual(got, b'??:??:??')
...
#                     self.expect_file('%s}/escaped.evil' % (outerdir_stripped))
#                 else:
...
def test_main():
    ...
    tests = [
        ...
        DetectReadTest,
        MemberReadTest,
        # FIX: Commented out because it is currently broken
        # GNUReadTest,
        # PaxReadTest,
        ListTest,
        WriteTest,
